...
export async function scrapeMerchantProfile(payload) {
  ...
  // Use OpenRouter to extract JSON via our existing generic generateOffer client
  // but using a specialized prompt for extraction. For simplicity, we just 
  // construct a basic fetch if generateOffer is strictly tied to offer prompts.
  let extracted = {};
  try {
    const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        ...
        ...(config.openRouterProxySessionToken ? { "Authorization": `Bearer ${config.openRouterProxySessionToken}` } : {}),
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "Extract the merchant's category, address, and business_hours from the search results. Return ONLY valid JSON matching this schema: {\"name\": \"string\", \"category\": \"string\", \"address\": \"string\", \"business_hours\": {\"Monday\": \"string\"}}",
          },
          {
            role: "user",
