...
const { ClientBrain, SecureStore } = require("../../../packages/intent-engine/src/modules/user/index.js");
...
function resolveOnnxModelPath() {
  const configuredPath = process.env.INTENT_ONNX_MODEL_PATH;
  if (configuredPath && configuredPath.trim().length) {
    return resolve(process.cwd(), configuredPath);
  }
  return resolve(process.cwd(), "packages/intent-engine/models/intent/intent_classifier.onnx");
}

function buildIntentRuntimePlan() {
  const onnxModelPath = resolveOnnxModelPath();
  const onnxConfigured = existsSync(onnxModelPath);
  const provider = onnxConfigured ? "onnx" : "ollama";
  return {
    provider,
    onnxConfigured,
    onnxModelPath,
    ollamaModel: process.env.OLLAMA_INTENT_MODEL || "nemotron-3-nano:4b",
    ollamaBaseUrl: process.env.OLLAMA_BASE_URL || "http://127.0.0.1:11434",
  };
}

async function buildIntentPacketForScenario({ scenario, baseUrl, intentRuntime }) {
  const prevProvider = process.env.INTENT_MODEL_PROVIDER;
  const prevOnnxPath = process.env.INTENT_ONNX_MODEL_PATH;
  const prevOllamaUrl = process.env.OLLAMA_BASE_URL;
  const prevOllamaModel = process.env.OLLAMA_INTENT_MODEL;

  process.env.INTENT_MODEL_PROVIDER = intentRuntime.provider;
  process.env.OLLAMA_BASE_URL = intentRuntime.ollamaBaseUrl;
  process.env.OLLAMA_INTENT_MODEL = intentRuntime.ollamaModel;
  if (intentRuntime.provider === "onnx") {
    process.env.INTENT_ONNX_MODEL_PATH = intentRuntime.onnxModelPath;
  }
  ...
  try {
    ...
    if (prevProvider === undefined) delete process.env.INTENT_MODEL_PROVIDER;
    else process.env.INTENT_MODEL_PROVIDER = prevProvider;
    if (prevOnnxPath === undefined) delete process.env.INTENT_ONNX_MODEL_PATH;
    else process.env.INTENT_ONNX_MODEL_PATH = prevOnnxPath;
    if (prevOllamaUrl === undefined) delete process.env.OLLAMA_BASE_URL;
    else process.env.OLLAMA_BASE_URL = prevOllamaUrl;
    if (prevOllamaModel === undefined) delete process.env.OLLAMA_INTENT_MODEL;
    else process.env.OLLAMA_INTENT_MODEL = prevOllamaModel;
  }
}

function assertLocalIntentSource(intentPacket, intentRuntime) {
  const source = intentPacket?.intent_source;
  if (source !== "embedded_onnx" && source !== "local_small_model") {
    throw new Error(
      `Expected local intent source for GDPR edge mode; got "${source || "unknown"}" (provider ${intentRuntime.provider}).`
    );
  }

  if (intentRuntime.provider === "onnx" && source !== "embedded_onnx") {
    throw new Error(`ONNX is configured but intent source is "${source}", expected "embedded_onnx".`);
  }

  if (intentRuntime.provider === "ollama" && source !== "local_small_model") {
    throw new Error(`Ollama fallback is configured but intent source is "${source}", expected "local_small_model".`);
  }
}
...
async function main() {
  ...
  try {
    const intentRuntime = buildIntentRuntimePlan();
    await writeJson(resolve(runDir, "00.intent-runtime.plan.json"), intentRuntime);
    if (intentRuntime.provider === "ollama") {
      await ensureLocalIntentModel(runDir);
    } else {
      await writeJson(resolve(runDir, "00.local-intent-model.check.request.json"), {
        skipped: true,
        reason: "onnx_configured_as_primary",
      });
      await writeJson(resolve(runDir, "00.local-intent-model.check.response.json"), {
        skipped: true,
        reason: "onnx_configured_as_primary",
      });
    }
    ...
    for (const scenario of scenarios) {
      const intentPacket = await buildIntentPacketForScenario({ scenario, baseUrl, intentRuntime });
      assertLocalIntentSource(intentPacket, intentRuntime);
      ...
      scenarioSummaries.push({
        ...
        user_pseudonym: scenario.user_pseudonym,
        intent_provider: intentRuntime.provider,
        intent_source: intentPacket.intent_source || null,
        intent_model: intentPacket.intent_model || null,
