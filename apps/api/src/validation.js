...
const internalMerchantRecord = vObject({
  ...
  approval_status: optional(vEnum(["pending", "approved", "rejected"])),
  approval_notes: optional(vString()),
  challenge_target_fit: optional(vBoolean()),
});

const voiceIdentitySchema = vObject({
  brand_story: vString({ minLength: 1, maxLength: 1200 }),
  menu_highlights: vArray(vString({ minLength: 1, maxLength: 140 }), { maxLength: 12 }),
  promotions: vArray(vString({ minLength: 1, maxLength: 140 }), { maxLength: 12 }),
  voice_name: vString({ minLength: 1, maxLength: 80 }),
  voice_id: vString({ minLength: 1, maxLength: 120 }),
  tone: vString({ minLength: 1, maxLength: 80 }),
  language: vString({ minLength: 2, maxLength: 32 }),
});
...
export const schemas = {
  health_output: vObject({
    ...
    intent_provider: optional(vString()),
    intent_model: optional(vString()),
    intent_mode: optional(vEnum(["edge_local_only", "remote_allowed"])),
  }),
  ...
  intent_signal_input: vObject({
    ...
    intent_source: optional(vEnum(["embedded_onnx", "local_small_model", "heuristic_fallback"])),
    intent_model: optional(vString({ minLength: 1 })),
  }),
  ...
  wallet_savings_output: vObject({
    user_pseudonym: vString({ minLength: 1 }),
    latest_saved_eur: vNumber({ min: 0 }),
    today_saved_eur: vNumber({ min: 0 }),
    month_saved_eur: vNumber({ min: 0 }),
    spots_tried: vNumber({ min: 0, integer: true }),
    updated_at_utc: vString({ pattern: isoDate }),
  }),
  ...
  voice_agent_identity_create_input: vObject({
    brand_story: optional(vString({ minLength: 1, maxLength: 1200 })),
    menu_highlights: optional(
      vArray(vString({ minLength: 1, maxLength: 140 }), { maxLength: 12 })
    ),
    promotions: optional(vArray(vString({ minLength: 1, maxLength: 140 }), { maxLength: 12 })),
    voice_name: optional(vString({ minLength: 1, maxLength: 80 })),
    voice_id: optional(vString({ minLength: 1, maxLength: 120 })),
    tone: optional(vString({ minLength: 1, maxLength: 80 })),
    language: optional(vString({ minLength: 2, maxLength: 32 })),
  }),
  voice_agent_identity_patch_input: vObject({
    brand_story: optional(vString({ minLength: 1, maxLength: 1200 })),
    menu_highlights: optional(
      vArray(vString({ minLength: 1, maxLength: 140 }), { maxLength: 12 })
    ),
  ...
  voice_agent_identity_output: vObject({
    merchant_id: vString({ minLength: 1 }),
    status: vEnum(["saved", "not_configured"]),
    updated_at_utc: vString({ pattern: isoDate }),
    identity: optional(voiceIdentitySchema),
  }),
  voice_agent_session_token_input: vObject({}),
  voice_agent_session_token_output: vObject({
    merchant_id: vString({ minLength: 1 }),
    agent_id: vString({ minLength: 1 }),
    provider: vEnum(["elevenlabs"]),
    created_at_utc: vString({ pattern: isoDate }),
    expires_at_utc: optional(vString({ pattern: isoDate })),
    session_token: optional(vString({ minLength: 1 })),
    signed_url: optional(vString({ minLength: 1 })),
  }),
  ...
  merchant_onboarding_scrape_profile_input: vObject({
    google_maps_url: vString({ minLength: 1 }),
  }),
  ...
  merchant_onboarding_scrape_profile_output: vObject({
    name: vString({ minLength: 1 }),
    category: vString({ minLength: 1 }),
    address: vString({ minLength: 1 }),
    phone: vString({ minLength: 1 }),
  ...
  internal_merchant_approval_input: vObject({
    merchant_id: vString({ minLength: 1 }),
    decision: vEnum(["approved", "rejected"]),
    reviewed_by: optional(vString({ minLength: 1 })),
    notes: optional(vString()),
  }),
  internal_merchant_approval_output: vObject({
    merchant_id: vString({ minLength: 1 }),
    approval_status: vEnum(["approved", "rejected"]),
    approved_by: vString({ minLength: 1 }),
    updated_at_utc: vString({ pattern: isoDate }),
  }),
};
