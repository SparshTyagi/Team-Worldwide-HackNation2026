...
export async function createRules(payload) {
  ...
  if (!uuid) {
    throw new Error(`Merchant not found: ${payload.merchant_id}. Seed it first via POST /internal/merchants.`);
  }
...
export async function dashboardOverview(merchantId) {
  ...
  if (!uuid) {
    return {
      merchant_id: merchantId,
      kpis: { offers_generated: 0, acceptance_rate: 0, redemption_rate: 0, estimated_net_uplift_eur: 0 },
      as_of_utc: nowIso(),
    };
  }
...
export async function dashboardFunnel(merchantId) {
  ...
  if (!uuid) {
    return {
      merchant_id: merchantId,
      funnel: { delivered: 0, viewed: 0, accepted: 0, redeemed: 0 },
      as_of_utc: nowIso(),
    };
  }
