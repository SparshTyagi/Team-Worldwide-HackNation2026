...
async function resolvePseudonym({ userPseudonym, authUserId }) {
  if (typeof userPseudonym === "string" && userPseudonym.trim().length > 0) {
    return userPseudonym.trim();
  }
  if (!authUserId) {
    throw new Error("user_pseudonym is required when auth_user_id is missing.");
  }
  ...
  if (!profile?.pseudonym) {
    throw new Error("No consumer pseudonym found for authenticated user.");
  }
...
export async function ingestIntent(payload) {
  ...
  const localityWithSignals = {
    ...
    intent_source: payload.intent_source || null,
    intent_model: payload.intent_model || null,
  };
...
export async function listActiveOffers(query) {
  const user = await resolvePseudonym({
    userPseudonym: query.user_pseudonym,
    authUserId: query.auth_user_id,
  });
...
export async function generateOfferForIntent({ intentPacket, channel = "in_app", locality }) {
  ...
  const approvedMerchants = (allMerchants || []).filter(
    (merchant) => (merchant.business_hours?.approval_status || "approved") === "approved",
  );
  const merchantPool = approvedMerchants.length ? approvedMerchants : allMerchants || [];
  ...
  const pool = candidates.length ? candidates : merchantPool;
  const ranked = pool
    .map((merchant) => ({ merchant, score: scoreMerchantForIntent(merchant, intentPacket) }))
    .sort((a, b) => b.score - a.score);
  const selectedMerchant = ranked[0]?.merchant || merchantPool?.[0];
  if (!selectedMerchant) {
...
export async function getCashback(userPseudonym) {
  const resolvedPseudonym = await resolvePseudonym({ userPseudonym });
  const { data: user } = await supabase
    .from("users")
    .select("id")
    .eq("pseudonym", resolvedPseudonym)
    .maybeSingle();

  if (!user) {
    return { user_pseudonym: resolvedPseudonym, cashback_balance_eur: 0, updated_at_utc: nowIso() };
  }
  ...
  return {
    user_pseudonym: resolvedPseudonym,
    cashback_balance_eur: Number(total.toFixed(2)),
    updated_at_utc: nowIso(),
  };
}

export async function getSavingsSummary(userPseudonym) {
  const resolvedPseudonym = await resolvePseudonym({ userPseudonym });
  const { data: user } = await supabase
    .from("users")
    .select("id")
  ...
  const offerIds = [...new Set(redemptions.map((row) => row.offer_id).filter(Boolean))];
  const { data: offers } = await supabase
    .from("offers")
    .select("id, merchant_id, discount_type, discount_value")
    .in("id", offerIds);

  const merchantIds = [...new Set((offers || []).map((row) => row.merchant_id).filter(Boolean))];
  ...
  for (const [index, redemption] of redemptions.entries()) {
    const offer = offerById.get(redemption.offer_id);
    if (!offer) continue;
    const merchantMeta = merchantById.get(offer.merchant_id)?.business_hours || {};
    const savedAmount = computeBudgetDeduction({ offer, merchantMeta });
    if (index === 0) latest = savedAmount;
    const redeemedAt = redemption.redeemed_at ? new Date(redemption.redeemed_at) : null;
    if (!redeemedAt || Number.isNaN(redeemedAt.getTime())) continue;
    if (
      redeemedAt.getUTCFullYear() === nowYear &&
      redeemedAt.getUTCMonth() === nowMonth
    ) {
      month += savedAmount;
      if (redeemedAt.getUTCDate() === nowDate) {
        today += savedAmount;
      }
    }
  }
  ...
  return {
    user_pseudonym: resolvedPseudonym,
    latest_saved_eur: Number(latest.toFixed(2)),
    today_saved_eur: Number(today.toFixed(2)),
    month_saved_eur: Number(month.toFixed(2)),
    spots_tried: redemptions.length,
    updated_at_utc: nowIso(),
  };
}
