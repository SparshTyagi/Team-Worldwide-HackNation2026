...
const LOCAL_INTENT_PROVIDERS = new Set(["onnx", "edge", "embedded", "ollama", "local"]);
...
async function callOllamaLocal({ baseUrl, model, systemPrompt, userPrompt, timeoutMs = 120000 }) {
...
function resolveIntentProvider(env = process.env) {
  return String(env.INTENT_MODEL_PROVIDER || "").trim().toLowerCase();
}

function shouldForceLocalIntentProvider(provider) {
  return LOCAL_INTENT_PROVIDERS.has(provider);
}
...
async function inferIntentWithLlm({
  ...
  const provider = resolveIntentProvider(env);
  const forceLocalOnly = shouldForceLocalIntentProvider(provider);
  const useOllama =
    forceLocalOnly ||
  ...
  if (useOllama) {
    ...
    try {
      ...
      const ollamaContent = completion?.message?.content;
      rawText =
        typeof ollamaContent === "string"
          ? ollamaContent.trim()
          : ollamaContent && typeof ollamaContent === "object"
            ? JSON.stringify(ollamaContent)
            : "";
      resolvedModel = model;
    } catch {
      if (forceLocalOnly) return null;
      throw new Error("Local intent model call failed");
    }
  } else {
    // In GDPR edge-first mode, remote intent inference is never allowed.
    if (forceLocalOnly) return null;
    const model =
