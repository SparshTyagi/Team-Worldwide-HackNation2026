...
const HAS_SUPABASE_ENV = Boolean(process.env.SUPABASE_URL && process.env.SUPABASE_KEY);
...
async function startServer() {
  const serverEntry = path.resolve(__dirname, "../../../../apps/api/src/index.js");
...
test(
  "client brain generates offer against live server",
  { skip: !HAS_SUPABASE_ENV },
  async () => {
...
test(
  "server offer lifecycle works end-to-end",
  { skip: !HAS_SUPABASE_ENV },
  async () => {
  const { server, baseUrl } = await startServer();
  try {
